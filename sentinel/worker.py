...
class MonitorWorker(threading.Thread):
    ...
    def _traffic_rate(self, counters):
        ...
        if self.previous_counters:
            elapsed = max(
                counters["timestamp"]
                - self.previous_counters["timestamp"],
                0.001,
            )
            rx_bps = max(
                (
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                rx_bps, tx_bps = self._traffic_rate(
                    counters
                )
                ...
                self.previous_connection_count = len(
                    connections
                )
...
class DiscoveryWorker(threading.Thread):
    ...
    def __init__(
        ...
        super().__init__(
            name="network-sentinel-discovery"
        )
        self.storage = storage
        self.interval = interval
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            ...
            # Wake early when the API asks for an immediate scan.
